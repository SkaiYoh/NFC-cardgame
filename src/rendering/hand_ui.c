...
Vector2 hand_ui_card_center_for_index(Rectangle handArea, int cardIndex) {
    ...
    const float runLength = (float)HAND_CARD_COUNT * (float)HAND_CARD_WIDTH
                          + (float)(HAND_CARD_COUNT - 1) * (float)HAND_CARD_GAP;
    ...
    center.y = startY + (float)cardIndex * stride;
    return center;
}
...
void hand_ui_draw(const Player *p, Texture2D placeholder) {
    ...
    for (int i = 0; i < HAND_CARD_COUNT; i++) {
        Vector2 center = hand_ui_card_center_for_index(p->handArea, i);
