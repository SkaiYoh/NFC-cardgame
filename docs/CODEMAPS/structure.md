...
│   ├── entities/       entity lifecycle, troop parsing, base implementation, projectile stub
...
| `src/entities/` | Entity allocation, state changes, troop parsing, base creation | `entities.c`, `troop.c`, `building.c` |
| `src/hardware/` | Serial NFC ingest from Arduino boards | `nfc_reader.c`, `arduino_protocol.c` |
| `src/logic/` | Game rules | `card_effects.c`, `combat.c`, `pathfinding.c`, `win_condition.c` |
...
| `tests/` | Headless executable tests | `test_pathfinding.c`, `test_combat.c`, `test_battlefield_math.c`, `test_battlefield.c`, `test_animation.c`, `test_debug_events.c`, `test_win_condition.c` |

## Build Surfaces

- `Makefile`
  - builds `cardgame`, the three tools, and all seven test executables
- `CMakeLists.txt`
...
- Because asset paths are relative, the game should be launched from the
  project root.
